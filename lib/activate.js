...
async function start(email, password, alf, verification, emailPassword) {
  ...
  try {
    ...
    const selectorCode = 'input[class="req"]';
    const needVerify = await page.$(selectorCode);
    ...
    if (needVerify) {
      ...
      await page.evaluate((text) => { (document.querySelector(selectorCode)).value = text; }, confirmNumber);
