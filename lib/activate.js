...
function getVerification(email, password, count = 0) {
  ...
  try {
    ...
    if (RETRY_COUNT !== count) {
      ...
      getVerification(email, password, count);
    }
  }
}
...
async function start(email, password, alf, verification, emailPassword) {
  ...
  try {
    ...
    let needVerify = await page.$('input[class="verify_code"]');

    if (needVerify) {
      console.log('[INFO] Detect verification code needed, retrieve it from Email...');
      const confirmNumber = verification || getVerification(email, emailPassword || password);

      await page.waitFor(1000);

      await page.evaluate((text) => { (document.querySelector('input[class="verify_code"]')).value = text; }, confirmNumber);
