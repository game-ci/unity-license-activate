...
async function start(email, password, alf, verification, emailPassword) {
  ...
  try {
    ...
    const conversations = await page.$('button[type=submit]');

    if (conversations) {
      await page.screenshot({ path: 'info_1.png' });
      start(email, password, alf, verification, emailPassword);
      await page.screenshot({ path: 'info_2.png' });
      return;
    }
